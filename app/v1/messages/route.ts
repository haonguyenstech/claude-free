...
export async function POST(request: Request) {
  ...
  let pending: { outputTokens: number; inputTokens?: number; ttftMs?: number; latencyMs?: number } | undefined;
  ...
  const backfill = (info: { outputTokens: number; inputTokens?: number; firstByteAt?: number; lastByteAt?: number }) => {
    ...
    if (logId != null) {
      try {
        updateRequestLogTokens(logId, info.outputTokens, info.inputTokens, ttftMs, latencyMs);
      } catch {}
    } else {
      pending = { outputTokens: info.outputTokens, inputTokens: info.inputTokens, ttftMs, latencyMs };
    }
  };

  const res = await routeMessages(areq, request.headers, request.signal, isStream ? backfill : undefined);
  ...
  try {
    ...
    if (pending && logId != null) {
      updateRequestLogTokens(logId, pending.outputTokens, pending.inputTokens, pending.ttftMs, pending.latencyMs);
    }
  } catch {}
